...
const DB_PATH     = path.join(__dirname, 'data', 'invite.db');
...
db.exec(`
  CREATE TABLE IF NOT EXISTS rsvps (
    ...
    attending   TEXT    NOT NULL DEFAULT 'both',
    message     TEXT,
...
const stmts = {
  insertRsvp: db.prepare(`
    INSERT INTO rsvps (name, email, phone, guests, attending, message)
    VALUES (@name, @email, @phone, @guests, @attending, @message)
  `),
  allRsvps: db.prepare(`
    SELECT * FROM rsvps ORDER BY created_at DESC
  `),
  rsvpById: db.prepare('SELECT * FROM rsvps WHERE id = ?'),
  rsvpCount: db.prepare('SELECT COUNT(*) as total, SUM(CAST(guests AS INTEGER)) as totalGuests FROM rsvps'),

  insertMessage: db.prepare(`
    INSERT INTO messages (name, relationship, message)
    VALUES (@name, @relationship, @message)
  `),
  approvedMessages: db.prepare(`
    SELECT id, name, relationship, message, created_at
    FROM messages WHERE approved = 1 ORDER BY created_at ASC
  `),
  allMessages: db.prepare('SELECT * FROM messages ORDER BY created_at DESC'),
  approveMsg: db.prepare('UPDATE messages SET approved = 1 WHERE id = ?'),
...
async function sendRsvpNotification(rsvp) {
  ...
  try {
    await transporter.sendMail({
      from: `"Evans Graduation Invite" <${process.env.SMTP_USER}>`,
      to:   NOTIFY_EMAIL,
...
async function sendConfirmationEmail(rsvp) {
  ...
  try {
    await transporter.sendMail({
      from: `"Evans Kioko Graduation" <${process.env.SMTP_USER}>`,
      to:   rsvp.email,
...
function fileFilter(allowedMimes) {
  return (req, file, cb) => {
    ...
    else cb(new Error(`Invalid file type: ${file.mimetype}. Allowed: ${allowedMimes.join(', ')}`));
  };
}
...
// POST /admin/upload/photo  — upload hero photo
  ...
  uploadPhoto(req, res, (err) => {
    ...
    try {
      fs.readdirSync(UPLOADS_DIR)
        .filter(f => f.startsWith('photo-') && f !== req.file.filename)
...
// POST /admin/upload/schedule — upload schedule PDF or doc
app.post('/admin/upload/schedule', requireAdmin, (req, res) => {
  req.uploadType = 'schedule';
  uploadSchedule(req, res, (err) => {
    if (err) return res.status(400).json({ ok: false, error: err.message });
    if (!req.file) return res.status(400).json({ ok: false, error: 'No file received' });
    try {
      fs.readdirSync(UPLOADS_DIR)
        .filter(f => f.startsWith('schedule-') && f !== req.file.filename)
        .forEach(f => fs.unlinkSync(path.join(UPLOADS_DIR, f)));
    } catch (_) {}
    res.json({ ok: true, url: `/uploads/${req.file.filename}`, filename: req.file.filename });
  });
...
// GET /api/assets — active photo & schedule URLs
app.get('/api/assets', (req, res) => {
  try {
    const files = fs.readdirSync(UPLOADS_DIR);
    const photoFile    = files.filter(f => /^photo-\d+\.(jpg|jpeg|png|webp)$/i.test(f)).sort().pop();
    const scheduleFile = files.filter(f => /^schedule-\d+\.(pdf|doc|docx)$/i.test(f)).sort().pop();
    res.json({
      ok: true,
      photoUrl:    photoFile    ? `/uploads/${photoFile}`    : null,
      scheduleUrl: scheduleFile ? `/uploads/${scheduleFile}` : '/schedule.pdf',
    });
  } catch (e) {
    res.json({ ok: true, photoUrl: null, scheduleUrl: '/schedule.pdf' });
  }
});
...
app.get('/api/messages', (req, res) => {
  try {
    const messages = stmts.approvedMessages.all();
...
app.post('/api/messages', (req, res) => {
  ...
  try {
    ...
    const created = { id: result.lastInsertRowid, name: sanitise(name)||'A Well-Wisher', relationship: sanitise(relationship)||'', message: sanitise(message) };
...
app.post('/api/rsvp', async (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM rsvps WHERE email = ?').get(email.trim().toLowerCase());
  if (existing) {
  ...
  try {
    ...
    const result = stmts.insertRsvp.run(rsvp);
    rsvp.id = result.lastInsertRowid;
...
app.get('/admin/rsvps', requireAdmin, (req, res) => {
  const rsvps = stmts.allRsvps.all();
  const stats = stmts.rsvpCount.get();
  res.json({ ok: true, stats, rsvps });
});

// GET /admin/messages — view all messages (including unapproved)
app.get('/admin/messages', requireAdmin, (req, res) => {
  const messages = stmts.allMessages.all();
  res.json({ ok: true, messages });
});

// PATCH /admin/messages/:id/approve
app.patch('/admin/messages/:id/approve', requireAdmin, (req, res) => {
  stmts.approveMsg.run(req.params.id);
  res.json({ ok: true });
});

// DELETE /admin/messages/:id
app.delete('/admin/messages/:id', requireAdmin, (req, res) => {
  stmts.deleteMsg.run(req.params.id);
  res.json({ ok: true });
});
...
app.listen(PORT, () => {
  ...
  ║  DB:    ${DB_PATH.slice(-30).padEnd(30)}  ║
  ║  Email: ${EMAIL_ENABLED ? 'ENABLED ✓' : 'DISABLED (set SMTP env vars)'}                     ║
  ╚══════════════════════════════════════════════════╝
  `);
});
